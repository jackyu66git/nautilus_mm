"""B3/S3 1R baseline. 15m OHLC. No EMA. No ATR. No fees."""
from __future__ import annotations

import pandas as pd

from chan_b3_baseline.paths import BAR_H, RISK_FRAC, TIME_BARS


def simulate_one(ev: dict, bar: pd.DataFrame, account: float) -> dict:
    i0 = int(ev["tape_row"])
    n = len(bar)
    kind = ev["kind"]
    buy = kind == "B3"
    ...
    base = {
        ...
        "t0_close_ts": str(bar.iloc[i0]["close_ts"]),
    }
    i1 = i0 + 1
    if i1 >= n:
    ...
    tp = entry + r_px if buy else entry - r_px
    qty = risk_usdt / r_px
